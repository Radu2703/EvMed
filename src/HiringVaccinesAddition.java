...
public class HiringVaccinesAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tff;
 private HiringVaccinesAdditionController c;
 private JOptionPane pab;
public HiringVaccinesAddition(String id, String cnpe)
{super("Vaccine Addition");
 JPanel p=new JPanel();
 ...
 p.add(tf10);
 c=new HiringVaccinesAdditionController();
 c.setID(id,cnpe);
 add=new JButton("Add vaccine");
...
class HiringVaccinesAdditionController implements ActionListener
{private JFrame his;
...
{String et=e.getActionCommand();
 if(et.equals("Add vaccine"))
{if(tf1.getText().trim().length()<=0||tff.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||tf9.getText().trim().length()<=0||tf10.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tff.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//")||tf8.getText().trim().contains("//")||tf9.getText().trim().contains("//")||tf10.getText().trim().contains("//"))
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    HiringVaccinesManager.memorize(fid,tf1.getText().trim(),tf2.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim());
    String message="You have registered a vaccine with the following data (ID, Medical File ID, Employee CNP, Type, Date, Building, Street, City, County, Doctor, Doctor Phone Number):\n".concat(HiringVaccinesManager.searchCNP_Fid_Date_Type(tf1.getText().trim(),fid,tf4.getText().trim(),tf2.getText().trim()));
    JOptionPane.showMessageDialog(pab, message,"Information",JOptionPane.INFORMATION_MESSAGE);
    if(his==null)his=new MedicalHistoryAddition(id,cnpe);
    his.pack();
    his.setLocation(500, 200);
    his.setVisible(true);}}}
