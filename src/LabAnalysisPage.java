...
import javax.swing.*;
public class LabAnalysisPage extends JFrame{
    ...
    private LabAnalysisController vac;
    private JTextField t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12;
    private JLabel title,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12;
public LabAnalysisPage(String c){
    super("Lab Analysis Page");
    cnp=c;
    p=new JPanel();
    vac=new LabAnalysisController();
    vac.setCNP(c);
    ...
    if(HiringLabAnalysisManager.countIDS(cnp))
       {String s=HiringLabAnalysisManager.displayIDs(cnp);
...
private class LabAnalysisController implements ActionListener{
...
{String et=e.getActionCommand();
 if(et.equals("Open file"))
    {if(t1.getText().trim().length()<=0||t2.getText().trim().length()<=0||t3.getText().trim().length()<=0||t4.getText().trim().length()<=0||t5.getText().trim().length()<=0||t6.getText().trim().length()<=0||t7.getText().trim().length()<=0||t8.getText().trim().length()<=0||t9.getText().trim().length()<=0||t10.getText().trim().length()<=0||t11.getText().trim().length()<=0||t12.getText().trim().length()<=0)
     ...
     else {info=HiringLabAnalysisManager.displayID(t1.getText().trim());
           infos=info.split("//");
           message="LAB ANALYSIS"+"\nID: "+infos[0]+"\nMedical File ID: "+infos[1]+"\nCNP: "+infos[2]+"\nType: "+infos[3]+"\nResults: "+infos[4]+"\nDate: "+infos[5]+"\nBuilding: "+infos[6]+"\nStreet: "+infos[7]+"\nCity: "+infos[8]+"\nCounty: "+infos[9]+"\nDoctor: "+infos[10]+"\nDoctor Phone Number: "+infos[11];
           title="LabAnalysis"+infos[0]+".txt";
           doc=new DocFrame(message,title);
           doc.pack();
           doc.setVisible(true);}
    }
 else if(e.getSource()==cb)
 {info=HiringLabAnalysisManager.displayID((String) cb.getSelectedItem());
  infos=info.split("//");
  t1.setText(infos[0]);
  t2.setText(infos[1]);
  t3.setText(infos[2]);
