...
public class PsychologyTestAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tfr;
 private PsychologyTestAdditionController c;
 private JOptionPane pab;
public PsychologyTestAddition()
{super("Psychology Test Addition");
 JPanel p=new JPanel();
 p.setLayout(new GridLayout(12, 2, 5, 1));
 ...
 p.add(tf10);
 c=new PsychologyTestAdditionController();
 add=new JButton("Add psychology test");
...
class PsychologyTestAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add psychology test"))
{if(tf1.getText().trim().length()<=0||tfr.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||tf9.getText().trim().length()<=0||tf10.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tf3.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//")||tf8.getText().trim().contains("//")||tf9.getText().trim().contains("//")||tf10.getText().trim().contains("//")||tfr.getText().trim().contains("//"))
 ...
 else if(!PsychologyTestManager.uniqueCNP_Date(tf1.getText().trim(),tf8.getText().trim()))
        JOptionPane.showMessageDialog(pab,"Error!\nThere is already a psychology test for the employee during that year!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(!EmployeeManager.bossRole(tf1.getText().trim()))
        JOptionPane.showMessageDialog(pab,"Error!\nThe employee is not in a leading role!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(!tfr.getText().trim().toLowerCase().equals("manager")&&!tfr.getText().trim().toLowerCase().equals("director")&&!tfr.getText().trim().toLowerCase().equals("department boss"))
        JOptionPane.showMessageDialog(pab,"Error!\nThe role must be manager, director, or department boss!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(!EmployeeManager.getRole(tf1.getText().trim()).toLowerCase().equals(tfr.getText().trim().toLowerCase()))
        JOptionPane.showMessageDialog(pab,"Error!\nThe employee has a different role from the one specified!", "Reminder", JOptionPane.WARNING_MESSAGE);
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    PsychologyTestManager.memorize(tf1.getText().trim(),tfr.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim());
    String message="You have registered a psychology test with the following data (ID, Employee CNP, Role, Type, Results, Date, Building, Street, City, County, Doctor, Doctor Phone Number):\n".concat(PsychologyTestManager.searchCNP_Date(tf1.getText(),tf4.getText()));
