...
public class HealthEvaluationAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tf11, tf12, tf13, tf14, tf15;
 private HealthEvaluationAdditionController c;
 private JOptionPane pab;
public HealthEvaluationAddition()
{super("Health Evaluation Addition");
 JPanel p=new JPanel();
 ...
 p.add(tf14);
 p.add(new Label("Doctor_Email"));
 tf15=new JTextField(10);
 p.add(tf15);
 c=new HealthEvaluationAdditionController();
 add=new JButton("Add health evaluation");
...
class HealthEvaluationAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add health evaluation"))
{if(tf1.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||
 ...
 else if(DoctorManager.uniqueEmail(tf15.getText().trim()))
        JOptionPane.showMessageDialog(pab,"Error!\nEmail of doctor not in the database!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(!HealthEvaluationManager.uniqueCNP_Date(tf1.getText().trim(),tf8.getText().trim()))
        JOptionPane.showMessageDialog(pab,"Error!\nThere is already a health evaluation for the employee during that year!", "Reminder", JOptionPane.WARNING_MESSAGE);
 ...
 else if(tf15.getText().trim().length()>50)
        JOptionPane.showMessageDialog(pab,"Error!\nDoctor email cannot have more than 50 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf15.getText().trim().contains("@")==false)
        JOptionPane.showMessageDialog(pab,"Error!\nDoctor email must have the @ character! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf15.getText().trim().startsWith("@")||tf4.getText().trim().endsWith("@"))
        JOptionPane.showMessageDialog(pab,"Error!\nDoctor email must have a name and a domain (characters before and after @)! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else{try 
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    HealthEvaluationManager.memorize(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim(),tf11.getText().trim(),tf12.getText().trim(),tf13.getText().trim(),tf14.getText().trim(),tf15.getText().trim());
    String message="You have registered a health evaluation with the following data (ID, Employee CNP, Medical history description, Recent medical events description, Blood pressure, Ophthalmological consultation, Pulse, Oxygen saturation, Date, Building, Street, City, County, Doctor, Doctor Phone Number, Doctor email):\n".concat(HealthEvaluationManager.searchCNP_Date(tf1.getText(),tf8.getText()));
    JOptionPane.showMessageDialog(pab, message,"Information",JOptionPane.INFORMATION_MESSAGE);
    AbilityFileManager.memorizeTest(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim(),tf11.getText().trim(),tf12.getText().trim(),tf13.getText().trim(),tf14.getText().trim(),tf15.getText().trim());
    message="You have registered an ability file with the following data (ID, Employee CNP, Medical history description, Recent medical events description, Blood pressure, Ophthalmological consultation, Pulse, Oxygen saturation, Date, Building, Street, City, County, Doctor, Doctor Phone Number, Result, Doctor email):\n".concat(AbilityFileManager.searchCNP_Date(tf1.getText(),tf8.getText()));
    JOptionPane.showMessageDialog(pab, message,"Information",JOptionPane.INFORMATION_MESSAGE);}
    }}
