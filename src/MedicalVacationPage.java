...
import javax.swing.*;
public class MedicalVacationPage extends JFrame{
    ...
    private MedicalVacationController vac;
...
public MedicalVacationPage(String c){
    super("Medical Vacation Page");
    cnp=c;
    p=new JPanel();
    vac=new MedicalVacationController();
    vac.setCNP(c);
    ...
    if(MedicalVacationManager.countIDS(cnp))
       {String s=MedicalVacationManager.displayIDs(cnp);
...
private class MedicalVacationController implements ActionListener{
    String info,cnp,message,title; String []infos=new String[14]; JFrame doc;
...
{String et=e.getActionCommand();
 if(et.equals("Open file"))
    {if(t1.getText().trim().length()<=0||t2.getText().trim().length()<=0||t3.getText().trim().length()<=0||t4.getText().trim().length()<=0||t5.getText().trim().length()<=0||t6.getText().trim().length()<=0||t7.getText().trim().length()<=0||t8.getText().trim().length()<=0||t9.getText().trim().length()<=0||t10.getText().trim().length()<=0||t11.getText().trim().length()<=0||t12.getText().trim().length()<=0||t13.getText().trim().length()<=0||t14.getText().trim().length()<=0)
     ...
     else {info=MedicalVacationManager.displayID(t1.getText().trim());
           infos=info.split("//");
           message="MEDICAL VACATION"+"\nID: "+infos[0]+"\nCNP: "+infos[1]+"\nBoss Email: "+infos[2]+"\nStart Date: "+infos[3]+"\nEnd Date: "+infos[4]+"\nPeriod: "+infos[5]+"\nReason: "+infos[6]+"\nBuilding: "+infos[7]+"\nStreet: "+infos[8]+"\nCity: "+infos[9]+"\nCounty: "+infos[10]+"\nDoctor: "+infos[11]+"\nDoctor Phone Number: "+infos[12]+"\nApproval: "+infos[13];
           title="MedicalVacation"+infos[0]+".txt";
