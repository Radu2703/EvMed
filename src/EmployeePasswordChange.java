...
import javax.swing.*;
public class EmployeePasswordChange extends JFrame{
    private String cnp;
    private JPanel p1;
    ...
    private EmployeePasswordChangeController cont;
public EmployeePasswordChange(String c){
    super("Employee Password Change");
    cnp=c;
    cont=new EmployeePasswordChangeController();
    cont.setCNP(c);
...
class EmployeePasswordChangeController implements ActionListener
{private String cnp;
...
public void actionPerformed(ActionEvent e){
  ...
  if(et.equals("Change password")){
    ...
    else if(EmployeeManager.uniqueCNP(cnp))
        JOptionPane.showMessageDialog(pab,"Error!\nCNP is not in the database!", "Reminder", JOptionPane.WARNING_MESSAGE);
    else if(EmployeeManager.uniqueEmail(tf1.getText().trim()))
        JOptionPane.showMessageDialog(pab,"Error!\nEmail is not in the database!", "Reminder", JOptionPane.WARNING_MESSAGE);
    else if(!EmployeeManager.emailPass(tf1.getText().trim()).equals(tf2.getText().trim()))
    ...
    {EmployeeManager.modifyPassword(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim());
     String message="The employee with email ".concat(EmployeeManager.searchEmailbyCNP(cnp).concat(" now has password ").concat(EmployeeManager.searchPasswordbyCNP(cnp)));
