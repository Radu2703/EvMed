...
public class RisksFileAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tff, tfd;
 private RisksFileAdditionController c;
 private JOptionPane pab;
public RisksFileAddition()
{super("Risks Data Addition");
 JPanel p=new JPanel();
 p.setLayout(new GridLayout(13, 2, 5, 1));
 p.add(new Label("Employee CNP"));
 tf1=new JTextField(10);
 p.add(tf1);
 p.add(new Label("File ID"));
 tff=new JTextField(10);
 p.add(tff);
 p.add(new Label("Description"));
 tf2=new JTextField(10);
 p.add(tf2);
 p.add(new Label("Chance"));
 ...
 p.add(tf10);
 c=new RisksFileAdditionController();
 add=new JButton("Add risks file data");
...
class RisksFileAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add risks file data"))
{if(tf1.getText().trim().length()<=0||tff.getText().trim().length()<=0||tfd.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||tf9.getText().trim().length()<=0||tf10.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tf3.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//")||tf8.getText().trim().contains("//")||tf9.getText().trim().contains("//")||tf10.getText().trim().contains("//")||tff.getText().trim().contains("//")||tfd.getText().trim().contains("//"))
        ...
        JOptionPane.showMessageDialog(pab,"Error!\nFile ID cannot have more than 11 digits! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf2.getText().trim().length()>100)
        JOptionPane.showMessageDialog(pab,"Error!\nDescription cannot have more than 100 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf3.getText().trim().length()>50)
        JOptionPane.showMessageDialog(pab,"Error!\nChance cannot have more than 50 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf4.getText().trim().length()>50)
        JOptionPane.showMessageDialog(pab,"Error!\nRecommendations cannot have more than 50 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 ...
 else if(tfd.getText().trim().length()!=10)
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    Integer yy = Integer.valueOf(tfd.getText().trim().substring(0, 4));
    char l1 = tfd.getText().trim().charAt(4);
    Integer mm = Integer.valueOf(tfd.getText().trim().substring(5, 7));
    char l2 = tfd.getText().trim().charAt(7);
    Integer dd = Integer.valueOf(tfd.getText().trim().substring(8, 10));
    if(mm<1||mm>12)
    ...
    RisksFileManager.memorize(tf1.getText().trim(),fid,tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tfd.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim());
    String message="You have registered a risk with the following data (ID, Employee CNP, Description, Chance, Recommendations, Date, Building, Street, City, County, Doctor, Doctor Phone Number):\n".concat(RisksFileManager.searchCNP_FID_Desc(tf1.getText(),fid,tf2.getText()));
