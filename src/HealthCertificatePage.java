...
import javax.swing.*;
public class HealthCertificatePage extends JFrame{
...
public HealthCertificatePage(String c){
    ...
    if(HealthCertificateManager.countIDS(cnp))
       {String s=HealthCertificateManager.displayIDs(cnp);
...
private class HealthCertificateController implements ActionListener{
...
{String et=e.getActionCommand();
 if(et.equals("Open file"))
    {if(t1.getText().trim().length()<=0||t2.getText().trim().length()<=0||t3.getText().trim().length()<=0||t4.getText().trim().length()<=0||t5.getText().trim().length()<=0||t6.getText().trim().length()<=0||t7.getText().trim().length()<=0||t8.getText().trim().length()<=0||t9.getText().trim().length()<=0||t10.getText().trim().length()<=0||t11.getText().trim().length()<=0)
     ...
     else {info=HealthCertificateManager.displayID(t1.getText().trim());
           infos=info.split("//");
           message="HEALTH CERTIFICATE"+"\nID: "+infos[0]+"\nCNP: "+infos[1]+"\nSickness: "+infos[2]+"\nRecommendations: "+infos[3]+"\nDate: "+infos[4]+"\nBuilding: "+infos[5]+"\nStreet: "+infos[6]+"\nCity: "+infos[7]+"\nCounty: "+infos[8]+"\nDoctor: "+infos[9]+"\nDoctor Phone Number: "+infos[10];
           title="HealthCertificate"+infos[0]+".txt";
           doc=new DocFrame(message,title);
           doc.pack();
           doc.setVisible(true);}
    }
 else if(e.getSource()==cb)
 {info=HealthCertificateManager.displayID((String) cb.getSelectedItem());
  infos=info.split("//");
  t1.setText(infos[0]);
  t2.setText(infos[1]);
  t3.setText(infos[2]);
