...
public class HealthCertificateAddition extends JFrame 
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10;
 private HealthCertificateAdditionController c;
 private JOptionPane pab;
public HealthCertificateAddition()
{super("Health Certificate Addition");
 JPanel p=new JPanel();
 p.setLayout(new GridLayout(11, 2, 5, 1));
 ...
 p.add(tf10);
 c=new HealthCertificateAdditionController();
 add=new JButton("Add health certificate");
...
class HealthCertificateAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add health certificate"))
{if(tf1.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||tf9.getText().trim().length()<=0||tf10.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tf3.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//")||tf8.getText().trim().contains("//")||tf9.getText().trim().contains("//")||tf10.getText().trim().contains("//"))
        ...
        JOptionPane.showMessageDialog(pab,"Error!\nSickness cannot have more than 100 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf3.getText().trim().length()>50)
        JOptionPane.showMessageDialog(pab,"Error!\nRecommendations cannot have more than 100 characters! ", "Reminder", JOptionPane.WARNING_MESSAGE);
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    HealthCertificateManager.memorize(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim());
    String message="You have registered a health certificate with the following data (ID, Employee CNP, Sickness, Recommendations, Date, Building, Street, City, County, Doctor, Doctor Phone Number):\n".concat(HealthCertificateManager.searchCNP_Date_Sickness(tf1.getText().trim(),tf4.getText().trim(),tf2.getText().trim()));
