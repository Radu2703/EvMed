...
public class DoctorAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7;
 private DoctorAdditionController c;
 private JOptionPane pab;
public DoctorAddition()
{super("Doctor Addition");
 JPanel p=new JPanel();
 ...
 p.add(tf7);
 c=new DoctorAdditionController();
...
class DoctorAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add doctor"))
{if(tf1.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0)
 ...
 else if(DoctorManager.uniqueCNP(tf1.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an doctor with the same CNP!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(DoctorManager.uniqueEmail(tf4.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an doctor with the same email!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(DoctorManager.uniquePhoneNumber(tf7.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an doctor with the same phone number!", "Reminder", JOptionPane.WARNING_MESSAGE);
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    DoctorManager.memorize(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),password,tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(), gender);
    String message="You have registered a doctor with the following data (CNP, Surname, Name, Email, Password, County, City, Phone Number, Gender):\n".concat(DoctorManager.searchCNP(tf1.getText()));
