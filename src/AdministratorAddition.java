...
public class AdministratorAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7;
 private AdministratorAdditionController c;
 private JOptionPane pab;
public AdministratorAddition()
{super("Administrator Addition");
 ...
 p.add(tf7);
 c=new AdministratorAdditionController();
 add=new JButton("Add administrator");
...
class AdministratorAdditionController implements ActionListener
{@Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add administrator"))
{if(tf1.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tf3.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//"))
        JOptionPane.showMessageDialog(pab,"Error!\nYou cannot enter \"//\" in the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 ...
 else if(AdministratorManager.uniqueCNP(tf1.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an administrator with the same CNP!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(AdministratorManager.uniqueEmail(tf4.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an administrator with the same email!", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(AdministratorManager.uniquePhoneNumber(tf7.getText().trim())==false)
        JOptionPane.showMessageDialog(pab,"Error!\nYou have already entered an administrator with the same phone number!", "Reminder", JOptionPane.WARNING_MESSAGE);
 ...
 else if(tf7.getText().trim().length()!=10)
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    long pnca=pnc/10;
    long pn=Long.parseLong(tf7.getText().trim());
    ...
    AdministratorManager.memorize(tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),password,tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),gender);
    String message="You have registered an administrator with the following data (CNP, Surname, Name, Email, Password, County, City, Phone Number, Gender):\n".concat(AdministratorManager.searchCNP(tf1.getText()));
