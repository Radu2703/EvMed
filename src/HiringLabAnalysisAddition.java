...
public class HiringLabAnalysisAddition extends JFrame
{private JButton add, cancel;
 private JTextField tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tff;
 private HiringLabAnalysisAdditionController c;
 private JOptionPane pab;
public HiringLabAnalysisAddition(String id, String cnpe)
{super("Lab Analysis Addition");
 JPanel p=new JPanel();
 ...
 p.add(tf10);
 c=new HiringLabAnalysisAdditionController();
 c.setID(id,cnpe);
 add=new JButton("Add lab analysis");
...
class HiringLabAnalysisAdditionController implements ActionListener
{private JFrame vac;
 private String id, cnpe;
 public void setID(String i, String c){id=i; cnpe=c;}
 @Override
public void actionPerformed(ActionEvent e)
{String et=e.getActionCommand();
 if(et.equals("Add lab analysis"))
{if(tf1.getText().trim().length()<=0||tff.getText().trim().length()<=0||tf2.getText().trim().length()<=0||tf3.getText().trim().length()<=0||tf4.getText().trim().length()<=0||tf5.getText().trim().length()<=0||tf6.getText().trim().length()<=0||tf7.getText().trim().length()<=0||tf8.getText().trim().length()<=0||tf9.getText().trim().length()<=0||tf10.getText().trim().length()<=0)
        JOptionPane.showMessageDialog(pab,"Error!\nYou need to fill the fields! ", "Reminder", JOptionPane.WARNING_MESSAGE);
 else if(tf1.getText().trim().contains("//")||tf2.getText().trim().contains("//")||tf3.getText().trim().contains("//")||tf4.getText().trim().contains("//")||tf5.getText().trim().contains("//")||tf6.getText().trim().contains("//")||tf7.getText().trim().contains("//")||tf8.getText().trim().contains("//")||tf9.getText().trim().contains("//")||tf10.getText().trim().contains("//")||tff.getText().trim().contains("//"))
   ...
   {long pnc=Long.parseLong(tf1.getText().trim());
    ...
    HiringLabAnalysisManager.memorize(fid,tf1.getText().trim(),tf2.getText().trim(),tf3.getText().trim(),tf4.getText().trim(),tf5.getText().trim(),tf6.getText().trim(),tf7.getText().trim(),tf8.getText().trim(),tf9.getText().trim(),tf10.getText().trim());
    String message="You have registered a lab analysis with the following data (ID, Medical File ID, Employee CNP, Type, Results, Date, Building, Street, City, County, Doctor, Doctor Phone Number):\n".concat(HiringLabAnalysisManager.searchCNP_Fid_Date_Type(tf1.getText().trim(),fid,tf4.getText().trim(),tf2.getText().trim()));
    JOptionPane.showMessageDialog(pab, message,"Information",JOptionPane.INFORMATION_MESSAGE);
    if(vac==null)vac=new HiringVaccinesAddition(id,cnpe);
    vac.pack();
    vac.setLocation(500, 200);
    vac.setVisible(true);}}}
